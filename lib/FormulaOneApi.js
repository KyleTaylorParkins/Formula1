...
module.exports = class FormulaOneApi {
    ...
    async getNextRace() {
        if (this.cachedNextRace && (this.cachedNextRace.refreshTime - Date.now() >= CACHE_TIMEOUT)) {
            console.log("Updating next race cache");
            ...
            console.log("Next race cache still valid, returning cache");
        }
