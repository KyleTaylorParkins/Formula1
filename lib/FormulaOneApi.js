...
module.exports = class FormulaOneApi {
    constructor(test) {
        ...
        this.log(`Created new Jolpica F1 API, test mode: ${this._test}`);
    }
    ...
    async apiCall(path) {
        const server = this._test ? '192.168.22.158' : 'api.jolpi.ca';
        const url = `https://${server}/ergast/f1/${path}.json`;
    ...
    async getNextRace() {
        ...
        // Check if there is json with the required data
        if (json === null) return null;
        if (json.MRData.RaceTable.Races.length < 1) return null;
        ...
        this.cachedNextRace = {
            ...
            secondPractice: race.SecondPractice, // Might be empty
            thirdPractice: race.ThirdPractice, // Might be empty
    ...
    async getDriverStandings() {
        ...
        if (json === null) return;
        if (!json.hasOwnProperty("MRData")) return;
        if (!json.MRData.StandingsTable.StandingsLists[0]) return;
        ...
        if (this._test) { 
            results.forEach(driver => {
                console.log(`Driver ${driver.givenName} ${driver.familyName} is on position ${driver.position}`);
            });
        }
